...
## Vantage website-specific authority

This repository is the canonical Nift source for the Vantage Browser website.
Edit `content/`, `templates/`, `.nift/` and this handover. Do not hand-edit
generated HTML under `public/`.

The outer source repository uses branch `stage`. The nested `public/` deployment
repository uses branch `main`. Commit generated `public/main` first, then commit
outer `stage` so the Git link records the matching generated commit. Do not create
an outer `main` branch or a nested `public/stage` branch.

Vantage is pre-implementation. Public copy must describe direction and planned
work, never imply that downloads, releases, compatibility, performance results or
security guarantees exist before accepted product evidence supports them.

The site should remain visually distinct from the other Gantry project sites: a
near-black graphite field, warm ivory type, sharp vermilion/coral accents and a
lime status colour, strong vertical framing and a restrained browser-view motif.
Use no blue and no gradients. Preserve responsive layout, visible focus states,
semantic landmarks, reduced-motion handling and readable text at 200% zoom.
